...
//     public int[][] generate(int[] arr) { // generate sets[i][]: all the results of subsetSum(len == i) - subsetSum(len
//                                           // == n - i)
//         int n = arr.length;
...
//         int limit = 1 << n; // using bits to represent the combos: 1 means the number at that index is
//                             // chosen to form the subset
...
//             // instead of storing the subSum we store the difference to avoid binary search
//             // later
//             // (sub1 - rest1) - (sub2 - rest2) = (sub1 + rest2) - (sub2 + rest1) = what the
//             // prob is asking for
...
//         // int a = test.minimumDifference(aa);

//         int ab[][] = test.generate(aa);
